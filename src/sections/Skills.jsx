...
import AnimatedSection from '../components/AnimatedSection'; // Importamos la animación
...
function Skills() {
    ...
    return (
        <section id="skills" className="py-16 sm:py-20 bg-background-light dark:bg-background-dark">
            <AnimatedSection> {/* Aplicamos la animación al contenido */}
                <div className="container mx-auto px-6">
                    <SectionTitle title={skillsData.title} />

                    {/* Restauramos el grid de 3 columnas */}
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">

                        {/* --- COLUMNA 1: Estrategia & Negocio --- */}
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-md">
                            <SkillColumnIcon>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                            </SkillColumnIcon>
                            <h3 className="text-xl font-bold text-center text-text-dark dark:text-text-light mb-2">{skillsData.strategyTitle}</h3>
                            {/* AÑADIMOS LA DESCRIPCIÓN */}
                            <p className="text-sm text-center text-slate-500 dark:text-slate-400 mb-6">{skillsData.strategyDesc}</p>
                        ...
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-md">
                            ...
                            <h3 className="text-xl font-bold text-center text-text-dark dark:text-text-light mb-2">{skillsData.technicalTitle}</h3>
                            {/* AÑADIMOS LA DESCRIPCIÓN */}
                            <p className="text-sm text-center text-slate-500 dark:text-slate-400 mb-6">{skillsData.technicalDesc}</p>
                        ...
                        {/* --- COLUMNA 3: Herramientas & Plataformas --- */}
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-md">
                            <SkillColumnIcon>
                            ...
                            <h3 className="text-xl font-bold text-center text-text-dark dark:text-text-light mb-2">{skillsData.toolsTitle}</h3>
                            {/* AÑADIMOS LA DESCRIPCIÓN */}
                            <p className="text-sm text-center text-slate-500 dark:text-slate-400 mb-6">{skillsData.toolsDesc}</p>
                            <motion.div className="flex flex-wrap justify-center gap-2" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
